...
Merge two (point) matrices into a new matrix whose numCols is the sum of those of a and b.
Assumes a and b have the same number of rows.
====================*/
struct matrix *matrix_merge(struct matrix *a, struct matrix *b) {
  struct matrix *c = matrix_newMatrix(a->cols + b->cols, a->rows);
  matrix_concatenate(c, a);
  matrix_concatenate(c, b);
  return c;
}
