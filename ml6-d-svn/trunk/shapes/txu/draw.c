...
void render_curve1( struct matrix *points, double x1, double y1, 
                   double x2, double y2, 
                   double x3, double y3, 
                   double x4, double y4, double step) {
  double t;
/*
  struct matrix *h = make_besier();
...
*/
  for(t=0;t<=1;t+=step){
    add_edge(points,(pow(1-t,3))*x1+x2*3*t*pow(1-t,2)+x3*(1-t)*t*t*3+pow(t,3)*x4,y4*pow(1-t,3)+y2*3*t*pow(1-t,2)+y3*(1-t)*t*t*3+pow(t,3)*y4,0,(pow(1-(t+step),3))*x1+x2*3*(t+step)*pow(1-(t+step),2)+x3*(1-(t+step))*pow(t+step,2)*3+pow(t+step,3)*x4,y4*pow(1-(t+step),3)+y2*3*(t+step)*pow(1-(t+step),2)+y3*(1-(t+step))*pow(t+step,2)*3+pow(t+step,3)*y4,0);
  }
...
void render_curve( struct matrix *points, double x1, double y1, 
  ...
  */
  ...
  for(t=0;t<=1;t+=step){
    add_edge(points,(2*pow(t,3)-3*pow(t,2)+1)*x1+(pow(t,3)-2*pow(t,2)+t)*(x2-x1)+(-2*pow(t,3)+3*pow(t,2))*x3+(pow(t,3)-pow(t,2))*(x4-x3),(2*pow(t,3)-3*pow(t,2)+1)*y1+(pow(t,3)-2*pow(t,2)+t)*(y2-y1)+(-2*pow(t,3)+3*pow(t,2))*y3+(pow(t,3)-pow(t,2))*(y4-y3),0,(2*pow(t+step,3)-3*pow(t+step,2)+1)*x1+(pow(t+step,3)-2*pow(t+step,2)+t+step)*(x2-x1)+(-2*pow(t+step,3)+3*pow(t+step,2))*x3+(pow(t+step,3)-pow(t+step,2))*(x4-x3),(2*pow(t+step,3)-3*pow(t+step,2)+1)*y1+(pow(t+step,3)-2*pow(t+step,2)+t+step)*(y2-y1)+(-2*pow(t+step,3)+3*pow(t+step,2))*y3+(pow(t+step,3)-pow(t+step,2))*(y4-y3),0);
  }
