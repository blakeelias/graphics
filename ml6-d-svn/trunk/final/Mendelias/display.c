...
jdyrlandweaver
====================*/
void plot( screen s, color c, int x, int y, double z) {
  int newy = YRES - 1 - y;
  if (x >= 0 && x < XRES && newy >= 0 && newy < YRES && z >= s[x][newy].z) {
    c.z = z;
    s[x][newy] = c;
  }
  //else   // good for debugging, but gets annoying when rendering stuff
  //  fprintf(stderr, "Pixel not in dimensions.\n");
}
