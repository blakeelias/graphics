  ...
  for (frame = 0; frame < num_frames+1; frame++) {
	...
	case VARY:
	  ...
	  for (ff = 0; ff <= range_length; ff++) {
	    ...
	    printf("Frame %d:  val: %lf\n",
		   (int)op[i].op.vary.start_frame+ff,
		   knob_vals[(int)op[i].op.vary.start_frame+ff][symtab_index].s.value);
	  }
	  /*printf("Frame %d:  val: %lf\n",
		   (int)op[i].op.vary.start_frame+ff,
		   knob_vals[(int)op[i].op.vary.start_frame+ff][symtab_index].s.value);*/
	  for ( ; op[i].op.vary.start_frame + ff < num_frames &&
		  knob_vals[(int)op[i].op.vary.start_frame+ff][symtab_index].s.value == 0; ff++) {
	...
	case LIGHT:
	  printf("Light: %s at: %6.2f %6.2f %6.2f",
		 ...
		 op[i].op.light.c[2]);
	  light_location[0] = op[i].op.light.c[0];
	  light_location[1] = op[i].op.light.c[1];
	  light_location[2] = op[i].op.light.c[2];
	  break;
	case AMBIENT:
