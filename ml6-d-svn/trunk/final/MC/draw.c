...
void connect_dots(struct matrix *sphere_points, struct matrix *points, int nsteps_lat, int nsteps_lon) { //DORITO READY
  ...
  for (lon = 0; lon < nsteps_lon; lon++) {
	...
	add_polygon(points,
				sphere_points->m[0][(lon+1)*nsteps_lat+lat+1], sphere_points->m[1][(lon+1)*nsteps_lat+lat+1], sphere_points->m[2][(lon+1)*nsteps_lat+lat+1],
				sphere_points->m[0][(lon+1)*nsteps_lat+lat], sphere_points->m[1][(lon+1)*nsteps_lat+lat], sphere_points->m[2][(lon+1)*nsteps_lat+lat],
				sphere_points->m[0][lon*nsteps_lat+lat+1], sphere_points->m[1][lon*nsteps_lat+lat+1], sphere_points->m[2][lon*nsteps_lat+lat+1]);
    }
	add_polygon(points,
	...
	add_polygon(points,
				sphere_points->m[0][(lon+1)*nsteps_lat], sphere_points->m[1][(lon+1)*nsteps_lat], sphere_points->m[2][(lon+1)*nsteps_lat],
				sphere_points->m[0][(lon+1)*nsteps_lat+lat], sphere_points->m[1][(lon+1)*nsteps_lat+lat], sphere_points->m[2][(lon+1)*nsteps_lat+lat],
				sphere_points->m[0][lon*nsteps_lat], sphere_points->m[1][lon*nsteps_lat], sphere_points->m[2][lon*nsteps_lat]);
  }
  ...
  /*for (lat = 0; lat < nsteps_lat-1; lat++) {	
	add_polygon(points,
				sphere_points->m[0][lat], sphere_points->m[1][lat], sphere_points->m[2][lat],
				sphere_points->m[0][lon*nsteps_lat+lat], sphere_points->m[1][lon*nsteps_lat+lat], sphere_points->m[2][lon*nsteps_lat+lat],
				sphere_points->m[0][lon*nsteps_lat+lat+1], sphere_points->m[1][lon*nsteps_lat+lat+1], sphere_points->m[2][lon*nsteps_lat+lat+1]);
	add_polygon(points,
				sphere_points->m[0][lat+1], sphere_points->m[1][lat+1], sphere_points->m[2][lat+1],
				sphere_points->m[0][lat], sphere_points->m[1][lat], sphere_points->m[2][lat],
				sphere_points->m[0][lon*nsteps_lat+lat+1], sphere_points->m[1][lon*nsteps_lat+lat+1], sphere_points->m[2][lon*nsteps_lat+lat+1]);
}/*
/*
	add_polygon(points,
				sphere_points->m[0][nsteps_lat-1],sphere_points->m[1][nsteps_lat-1],sphere_points->m[2][nsteps_lat-1],
				sphere_points->m[0][(nsteps_lat*nsteps_lat)-1],sphere_points->m[1][(nsteps_lat*nsteps_lat)-1],sphere_points->m[2][(nsteps_lat*nsteps_lat)-1],
				sphere_points->m[0][(nsteps_lat*nsteps_lat-1)-1],sphere_points->m[1][(nsteps_lat*nsteps_lat-1)-1],sphere_points->m[2][(nsteps_lat*nsteps_lat-1)-1]);

	add_polygon(points,
				sphere_points->m[0][0],sphere_points->m[1][0],sphere_points->m[2][0],
				sphere_points->m[0][nsteps_lat-1],sphere_points->m[1][nsteps_lat-1],sphere_points->m[2][nsteps_lat-1],
				sphere_points->m[0][(nsteps_lat*nsteps_lat-1)-1],sphere_points->m[1][(nsteps_lat*nsteps_lat-1)-1],sphere_points->m[2][(nsteps_lat*nsteps_lat-1)-1]);
*/
}
...
double dfcw;	//distance from clockwise
  ...
  for (i=0; i < points->lastcol - 2; i+=3) {
	...
	if(dfcc<dfcw){
	  ...
	  //c.green=rand()%255;
	  //c.blue=rand()%255;
		shade_polygon(points->m[0][i], points->m[1][i], points->m[0][i+1], points->m[1][i+1], points->m[0][i+2], points->m[1][i+2],s, c);
		}
}	
}
...
//printf("shade poly passed teh obvious \n");
...
if(xlow==0&&xhigh==1){
pty=(((double)x2-(double)x0)*(((double)y1-(double)y0)/((double)x1-(double)x0)))+y0;
draw_line(x2,y2,x2,pty,s,c);
...
if(xlow==0&&xhigh==2){
pty=(((double)x1-(double)x0)*(((double)y2-(double)y0)/((double)x2-(double)x0)))+y0;
draw_line(x1,y1,x1,pty,s,c);
...
if(xlow==2&&xhigh==0){
pty=(((double)x1-(double)x2)*(((double)y0-(double)y2)/((double)x0-(double)x2)))+y2;
draw_line(x1,y1,x1,pty,s,c);
//shade_triangle(x0,y0,x1,y1,x1,pty,s,c);		
//shade_triangle(x2,y2,x1,y1,x1,pty,s,c);
return;
...
if(xlow==2&&xhigh==1){
pty=(((double)x0-(double)x2)*(((double)y1-(double)y2)/((double)x1-(double)x2)))+y2;
draw_line(x0,y0,x0,pty,s,c);
...
if(xlow==1&&xhigh==0){
pty=(((double)x2-(double)x1)*(((double)y0-(double)y1)/((double)x0-(double)x1)))+y1;
draw_line(x2,y2,x2,pty,s,c);
//shade_triangle(x0,y0,x2,y2,x2,pty,s,c);		
//shade_triangle(x1,y1,x2,y2,x2,pty,s,c);
return;
...
if(xlow==1&&xhigh==2){
pty=(((double)x0-(double)x1)*(((double)y2-(double)y1)/((double)x2-(double)x1)))+y1;
draw_line(x0,y0,x0,pty,s,c);
//shade_triangle(x1,y1,x0,y0,x0,pty,s,c);		
//shade_triangle(x2,y2,x0,y0,x0,pty,s,c);
return;
...
for(d=0;d<(x2-x0);++d){
draw_line(k,(int)tp,k,(int)bp,s,c);
tp+slope1;
bp+slope2;
k+=dir;
}
