...
void render_box(struct matrix *points, double x, double y, double z,
  ...
  add_edge(points, x, y2, z, x2, y2, z);
  add_edge(points, x2, y2, z, x2, y, z);
  add_edge(points, x2, y, z, x, y, z);

  add_edge(points, x, y, z2, x, y2, z2);
  add_edge(points, x, y2, z2, x2, y2, z2);
  add_edge(points, x2, y2, z2, x2, y, z2);
  add_edge(points, x2, y, z2, x, y, z2);

  add_edge(points, x, y, z, x, y, z2);
  add_edge(points, x, y2, z, x, y2, z2);
  add_edge(points, x2, y2, z, x2, y2, z2);
  add_edge(points, x2, y, z, x2, y, z2);
}
...
void render_torus_mesh( struct matrix *points, double r, double r2,
  ...
  generate_torus_mesh(temp, r, r2, offx, offy, offz, step);
...
void generate_sphere_mesh( struct matrix *points, double r,
  ...
  for( rt=0; rt <= 1; rt+=step) 
    for( c=0; c <= 1; c+=step ) {
      ...
      y = r * sin(2 * M_PI * c) + offy;
      z = r * -1 * cos(2 * M_PI * c) * sin(M_PI * rt) + offz;
      add_point(points, x, y, z);
    }
}
