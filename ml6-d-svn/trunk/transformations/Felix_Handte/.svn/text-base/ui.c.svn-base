...
static gint draw_area_clicked(GtkWidget *widget, gpointer user_data);
...
void initWindow(int *argc, char ***argv, int arg_width, int arg_height, char *arg_window_title){
	...
	gtk_widget_realize(eventbox); //         Required to get a valid X window ID
	...
	sprintf(SDL_windowid,"SDL_WINDOWID=%ld", GDK_WINDOW_XWINDOW(eventbox->window));
	putenv(SDL_windowid);
	...
	g_signal_connect(eventbox, "clicked", G_CALLBACK(draw_area_clicked), NULL);
	exit(0);
	atexit(SDL_Quit);
	...
	if(screen->format->BytesPerPixel != 4){
		fprintf(stderr, "Not in 32 bit color mode. Ain' gon' work.\n");
		exit(EXIT_FAILURE);
	}
...
static gint draw_area_clicked(GtkWidget *widget, gpointer user_data){
	//gtk_label_set_text(GTK_LABEL(curlabel), "CLICK");
